...
def move_clip_wrapper(row, label_to_dir, trim_format, src_dir):
    ...
    src_path = os.path.join(src_dir, row['video-id'] + '.mp4')

    shutil.copy(src_path, output_filename)

    status = tuple([clip_id, True, 'Moved'])
    return status
...
def main(input_csv,
    ...
    for _, row in dataset.iterrows():
        move_clip_wrapper(row, label_to_dir, trim_format, src_dir)
