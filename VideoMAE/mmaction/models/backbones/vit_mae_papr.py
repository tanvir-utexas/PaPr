...
from .x3d_proj import X3Dm_proj, X3Ds_proj
from .resnet3d_slowonly_proj import SlowOnly_proj
...
class PaPrVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        ## init patch projection x3d block
        if proposal == "x3ds":
            self.proposal = X3Ds_proj()
        elif proposal == "x3dm":
            self.proposal = X3Dm_proj()
        elif proposal == "slowonly_resnet50":
            self.proposal = SlowOnly_proj(model="res50")
        elif proposal == "slowonly_resnet101":
            self.proposal = SlowOnly_proj(model="res101")
        else:
            raise Exception("Sorry. Proposal model is not prepared.")
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.fraction != 1.0:
            with torch.no_grad():
                mask = self.proposal(x)
                n = int(mask.shape[-1] * self.fraction)
                mask = mask[:, :n]
        ...
        if self.fraction != 1.0:
            c = x.shape[-1]
            x = x.gather(dim=1, index=mask.expand(b, -1, c))
