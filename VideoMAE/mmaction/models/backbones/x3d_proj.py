...
class X3Dm_proj(nn.Module):
    ...
    def __init__(self,
                 ...
                 gamma_b=2.25,
                 gamma_d=2.2,
                #  pretrained="./checkpoints/x3d_s_13x6x1_facebook-kinetics400-rgb_20201027-623825a0.pth",
                 pretrained="./checkpoints/x3d_m_16x5x1_facebook-kinetics400-rgb_20201027-3f42382a.pth",
                 in_channels=3,
                 num_stages=4,
        ...
        self.avg_pool = nn.AvgPool3d((2, 1, 1), stride=(2, 1, 1))
        self.init_weights()
    ...
    def init_weights(self):
        ...
        if isinstance(self.pretrained, str):
            ...
            for k, v in state_dict.items():
                if 'backbone' in k:
                    revised_dict[k.replace('backbone.', '')] = v
            
            self.load_state_dict(revised_dict, strict=True)
...
class X3Ds_proj(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        if x.shape[-1] != 160:
            x = F.interpolate(x, (8, 160, 160), align_corners=True, mode="trilinear")
        x = self.conv1_s(x)
        x = self.conv1_t(x)
        for layer_name in self.res_layers:
            res_layer = getattr(self, layer_name)
