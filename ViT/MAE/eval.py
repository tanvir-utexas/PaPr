...
def main_worker(gpu, ngpus_per_node, args):
    ...
    elif args.vit_arch == "large":
        model_name = "vit_large_patch16"
    elif args.vit_arch == "huge":
        model_name = "vit_huge_patch14"

    args.model_path = f"./mae_weights/mae_finetuned_vit_{args.vit_arch}.pth"
    ...
    if os.path.isfile(args.model_path):
        print("=> loading pretrained checkpoint '{}'".format(args.model_path))
        if args.gpu is None:
    ...
    if args.z < 1.0:
        PaPr.create_papr_model(model, proposal_model=args.prop_arch, z=args.z, input_size=args.input_size, proposal_weights=args.proposal_weights)
    ...
    if args.profiling:
        if args.gpu == 0:
            flop = calc_flops(model.module, args.input_size)
            thr, memory = throughput(model.module, args.input_size, args.prof_batch_size)
